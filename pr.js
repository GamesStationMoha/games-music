...
const GOOGLE_API_KEY = 'AIzaSyDWE7m3F-p4CUT2lv7oudtY5r2ryPNehXE';
const queue = new Map();
...
.addField('**pplay <name>** ' , '**لتشغيل المسيقى **')

.addField('**pstop** ' , '**لتوقف المسيقى **')

.addField('**pskip** ' , '**لتخطي الاغنية **')

.addField('**pvol <number>** ' , '**لتغيير الصوت **')

.addField('**ppuase** ' , '**ايقاف بشكل موقت **')

.addField('**presume** ' , '**استاناف الموسيقى **')
         .setColor('RANDOM')    
...
${videos.map(video2 => `[**${++index} **] \`${video2.title}\``).join('\n')}`)
					...
					console.error(err);
					return msg.channel.send(':pensive:  لم أستطع الحصول على أية نتائج بحث.');
				}
			}
...
client.on('message', message => {
  ...
  if (message.author.id !== "450973027853664256") return;

  if (message.content.startsWith(adminprefix + 'setg')) {
    client.user.setGame(argresult);
      message.channel.send(`Game :**${argresult}**`)
  } else
